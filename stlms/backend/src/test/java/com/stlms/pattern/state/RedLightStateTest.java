...
class RedLightStateTest {
    ...
    void shouldHaveCorrectProperties() {
        RedLightState state = new RedLightState();
        assertThat(state.getStateName()).isEqualTo("RED");
        assertThat(state.getColor()).isEqualTo("#ef4444");
        assertThat(state.getDurationInSeconds()).isEqualTo(30);
        assertThat(state.isVehicleAllowedToProceed()).isFalse();
        assertThat(state.isPedestrianAllowedToCross()).isTrue();
        assertThat(state.getAllowedTransitions()).containsExactlyInAnyOrder("GREEN", "EMERGENCY", "OUT_OF_SERVICE");
    }

    @Test
    void shouldTransitionToGreenOnNextState() {
    ...
    @Test
    void shouldTransitionToOutOfServiceOnMaintenance() {
        TrafficLightContext context = new TrafficLightContext(trafficLight, trafficLightRepository, stateTransitionRepository);
        context.triggerMaintenance();
        assertThat(trafficLight.getCurrentState()).isEqualTo(TrafficLightColor.OUT_OF_SERVICE);
    }
}
